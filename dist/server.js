...
async function articleCTAReviewer(article) {
    ...
    try {
        const response = await axios.post(`${OLLAMA_URL}/api/generate`, {
            model: "phi3:14b-medium-128k-instruct-q4_K_S",
            prompt: `
      ROLE:
...
async function crawlWebsite(url) {
    ...
    const browser = await puppeteer.launch({
        headless: "new",
        executablePath: "/root/.cache/puppeteer/chrome/linux-137.0.7151.70/chrome-linux64/chrome/chrome/linux-137.0.7151.70/chrome-linux64/chrome",
...
cron.schedule("0 0 * * *", (0, asyncMiddleware_1.default)(async () => {
    ...
    console.log("[CRON] Running scheduled task at midnight 1", generateArticleWebMetrics);
    let content;
    try {
