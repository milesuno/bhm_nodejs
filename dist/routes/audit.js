...
router.post("/", (0, asyncMiddleware_1.default)(async (req, res) => {
    const { url } = req.body;
    let document;
    try {
        ...
        // **Extract Meta Tags**
        const metaTitle = await page.evaluate(() => document.querySelector("title")?.innerText || "❌ Missing");
        const metaDesc = await page.evaluate(() => document.querySelector('meta[name="description"]')?.content ||
            "❌ Missing");
        const metaDescLength = metaDesc.length >= 150 && metaDesc.length <= 160
            ? "✅ Good"
            : "⚠️ Too Long/Short";
        ...
        const seoChecks = await page.evaluate(() => {
            ...
            const imageAltCheck = imagesWithoutAlt > 0
        ...
        const trackingScripts = await page.evaluate(() => {
            ...
            const googleAnalytics = scripts.some((src) => src.includes("GoogleAnalyticsObject")) ||
                window?.ga ||
                window?.GoogleAnalyticsObject;
            const googleTagManager = scripts.some((src) => src.includes("googletagmanager.com/gtm.js")) || window?.dataLayer;
            const googleAnalytics4 = pageText.includes("gtag('config')") || window?.gtag;
            const adobeAnalytics = scripts.some((src) => src.includes("omniture") || src.includes("adobedtm.com")) || window?.s;
            const adobeTarget = scripts.some((src) => src.includes("tt.omtrdc.net")) ||
                window?._satellite;
            const tealium = scripts.some((src) => src.includes("tags.tiqcdn.com/utag")) ||
                window?.utag;
            const tiktokPixel = scripts.some((src) => src.includes("ticdn.com/pixel")) ||
                window?.tiktokPixel;
            const floodlight = scripts.some((src) => src.includes("fls.doubleclick.net")) ||
                window?.FLC;
            const crazyEgg = scripts.some((src) => src.includes("crazyegg.com")) || window?.CE;
            const hotjar = scripts.some((src) => src.includes("hotjar.com")) || window?.hj;
            const contentSquare = scripts.some((src) => src.includes("contentsquare.net")) ||
                window?._contentSquare;
            const oneTrust = scripts.some((src) => src.includes("onetrust.com") || src.includes("cookie-consent")) || window?.OneTrust;
            return {
        ...
        const consentBanner = await page.evaluate(() => {
            ...
            const allTexts = [...document.querySelectorAll("div, p, button")].map((el) => el.innerText.toLowerCase());
            return allTexts.some((text) => keywords.some((keyword) => text.includes(keyword)))
                ? "✅ Found"
                : "❌ Not Found";
        });
        await browser.close();
        const acceptConsentBanner = async (page) => {
            try {
                ...
                for (const frame of frames) {
                    try {
                        ...
                        const buttonExists = await frame.evaluate((selectors) => {
                            return selectors.some((selector) => document.querySelector(selector));
                        }, consentButtonSelectors);
                        if (buttonExists) {
                            console.log("✅ Found consent button inside iframe. Clicking...");
                ...
                if (!foundBanner) {
                    const buttonExists = await page.evaluate((selectors) => {
                        return selectors.some((selector) => document.querySelector(selector));
                    }, consentButtonSelectors);
                    if (buttonExists) {
                        console.log("✅ Found consent button on the main page. Clicking...");
                        await Promise.all([
